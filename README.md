...
| 챕터 | 항목                                                                                 | 작성자          |
| ---- | ------------------------------------------------------------------------------------ | --------------- |
| 4    | 항목20 값에 의한 전달보다는 상수객체 참조자에 의한 전달 방식을 택하는 편이 대개 낫다 | WooHyunKwon1012 |
|      | 항목21 함수에서 객체를 반환해야 할 경우에 참조자를 반환하려고 들지 말자              | SHIN-DONG-UK    |
|      | 항목22 데이터 멤버가 선언될 곳은 private 영역임을 명심하자                           | corlexvusR      |
|      | 항목23 멤버 함수보다는 비멤버 비프렌드 함수와 더 가까워지자                          | YE7283          |
|      | 항목24 타입 변환이 모든 매개변수에 대해 적용되어야 한다면 비멤버 함수를 선언하자     | Hi-MuMu         |
|      | 항목25 예외를 던지지 않는 swap에 대한 지원도 생각해 보자                             | han-skyler      |
...
|      | 항목28 내부에서 사용하는 객체에 대한 핸들을 반환하는 코드는 되도록 피하자 | corlexvusR      |
|      | 항목29 예외 안전성이 확보되는 그날 위해 싸우고 또 싸우자!                 | YE7283          |
|      | 항목30 인라인 함수는 미주알고주알 따져서 이해해 두자                      | Hi-MuMu         |
|      | 항목31 파일 사이의 컴파일 의존성을 최대로 줄이자                          | han-skyler      |
...
|      | 항목34 인터페이스 상속과 구현 상속의 차이를 제대로 파악하고 구별하자           | corlexvusR      |
|      | 항목35 가상 함수 대신 쓸 것들도 생각해 두는 자세를 시시때때로 길러 두자        | YE7283          |
|      | 항목36 상속받은 비가상 함수를 파생 클래스에서 재정의하는 것은 절대 금물!       | Hi-MuMu         |
|      | 항목37 어떤 함수에 대해서도 상속받은 기본 매개변수 값은 절대로 재정의하지 말자 | han-skyler      |
...
|      | 항목42 typename의 두 가지 의미를 제대로 파악하자                                  | Hi-MuMu         |
...
| 챕터 | 항목                                                                              | 작성자          |
| ---- | --------------------------------------------------------------------------------- | --------------- |
| 7    | 항목44 매개변수에 독립적인 코드는 템플릿으로부터 분리시키자                       | WooHyunKwon1012 |
|      | 항목45 호환되는 모든 타입을 받아들이는 데는 멤버 함수 템플릿이 직방!              | SHIN-DONG-UK    |
|      | 항목46 타입 변환이 바람직할 경우에는 비멤버 함수를 클래스 템플릿 안에 정의해 두자 | corlexvusR      |
